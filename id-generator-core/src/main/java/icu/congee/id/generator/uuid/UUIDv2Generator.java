 ...
 * UUIDv2生成器 (DCE安全规范实现)
 *
 * <p>该类用于生成符合DCE安全规范的UUIDv2。该版本包含POSIX用户/组ID信息， 主要应用于需要系统级安全标识的场景。使用时需注意平台兼容性。
 *
 * <p>UUIDv2结构规范：
 *
 * <ul>
...
public class UUIDv2Generator implements IdGenerator {
    ...
    public static UUID next() {
        ...
        // 处理时间回拨并获取序列号
        long sequence = timestampCounter.updateAndGet(prev -> 
            timestamp > prev ? timestamp : prev + 1
        );
